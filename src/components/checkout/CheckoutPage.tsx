...
interface CustomerInfo {
  name: string;
  phone: string;
  // Se eliminó email de la interfaz
}
...
export const CheckoutPage: React.FC<CheckoutPageProps> = ({
  ...
  // Log inicial de props recibidas
  console.log('[CheckoutPage] Props recibidas:', {
  ...
  const [customerInfo, setCustomerInfo] = useState<CustomerInfo>({
    name: '',
    phone: '',
    // Se eliminó email del estado inicial
  });
  ...
  useEffect(() => {
    if (initialCommerceData) {
      console.log('[CheckoutPage] Usando datos de comercio iniciales:', initialCommerceData);

      // Asegurarse de que los valores numéricos sean realmente números
      const processedCommerceData = {
        ...initialCommerceData,
        min_order_value: initialCommerceData.min_order_value ? Number(initialCommerceData.min_order_value) : 0,
        delivery_fee: initialCommerceData.delivery_fee ? Number(initialCommerceData.delivery_fee) : 0,
      };

      setCommerceData(processedCommerceData);
  ...
  // Log para monitorear los items cuando cambian
  useEffect(() => {
    console.log('[CheckoutPage] Items actualizados:', items.map(item => ({
      id: item.id,
      name: item.name,
      price: item.price,
      quantity: item.quantity,
  ...
  const fetchCommerceData = async () => {
    ...
    try {
      ...
      console.log('[CheckoutPage] Datos de comercio obtenidos de API:', response.data.commerce);

      // Procesar los datos para asegurar que los valores numéricos sean realmente números
      const processedCommerceData = {
        ...response.data.commerce,
        min_order_value: response.data.commerce.min_order_value ? Number(response.data.commerce.min_order_value) : 0,
        delivery_fee: response.data.commerce.delivery_fee ? Number(response.data.commerce.delivery_fee) : 0,
      };
  ...
  const calculateTotal = () => {
    // Log para rastrear llamadas a calculateTotal
    console.log('[CheckoutPage] Llamada a calculateTotal. Caller:', new Error().stack?.split('\n')[2]?.trim());

    const total = items.reduce((total, item) => {
      // Asegurarse de que price y quantity son números válidos
      const price = Number(item.price) || 0;
      const quantity = Number(item.quantity) || 1;

      console.log(`[CheckoutPage] Calculando precio para item ${item.name}:`, {
        price,
        quantity,
      ...
      if (item.selected_options) {
        item.selected_options.forEach(option => {
          option.selected_items.forEach(selectedItem => {
            // Asegurarse de que price_addition es un número válido
            const priceAddition = Number(selectedItem.price_addition) || 0;
            itemPrice += priceAddition * quantity;
          });
        });
      }
    ...
    console.log('[CheckoutPage] Total calculado:', total);
    return total;
  };
  ...
  const calculateShippingCost = () => {
    ...
    // Usar el delivery_fee del comercio si está disponible, asegurando que sea un número
    const fee = commerceData?.delivery_fee ? Number(commerceData.delivery_fee) : 0;
    console.log('[CheckoutPage] Costo de envío calculado:', fee, 'tipo:', typeof fee);
    return fee;
  };

  // Total final con costos de envío (sin impuestos)
  const finalTotal = calculateTotal() + calculateShippingCost();
  console.log('[CheckoutPage] Total final calculado:', finalTotal);

  // Formato para precios en ARS ($)
  const formatPrice = (price: number) => {
    // Asegurar que price es un número
    const numericPrice = Number(price);
    console.log('[CheckoutPage] Formateando precio:', numericPrice, 'tipo:', typeof numericPrice);

    // Si el valor parece estar en millones cuando debería estar en miles (para debugging)
    if (numericPrice > 1000000) {
      console.warn('[CheckoutPage] ⚠️ Valor monetario muy alto detectado:', numericPrice);
    }
    ...
    console.log('[CheckoutPage] Precio formateado:', formattedPrice);
  ...
  const validateForm = () => {
    ...
    if (deliveryMethod === 'delivery') {
      ...
      // Convertir explícitamente min_order_value a número para la comparación
      if (commerceData?.min_order_value && orderTotal < Number(commerceData.min_order_value)) {
        setErrorMessage(`El pedido mínimo para delivery es ${formatPrice(Number(commerceData.min_order_value))}`);
        return false;
      }
    }
  ...
  // Efecto para depurar renderizados y valores monetarios
  useEffect(() => {
    console.log('---- DEBUG VALORES MONETARIOS ----');
    console.log('Items (length):', items.length);
    items.forEach((item, idx) => {
      console.log(`Item ${idx}: ${item.name} - Precio: ${item.price} (${typeof item.price}) * Cantidad: ${item.quantity}`);
    });
    console.log('Subtotal calculado:', calculateTotal());
    console.log('Costo de envío:', calculateShippingCost());
    console.log('Total final:', finalTotal);
    console.log('---- FIN DEBUG ----');
  }, [items, deliveryMethod, finalTotal]);

  // Componente para el resumen en un modal
  const OrderSummaryContent = () => {
    // Log para depuración al renderizar el componente de resumen
    console.log('[OrderSummaryContent] Renderizando con valores:', {
          ...
          >
            ...
            <div className="flex-1">
              <div className="flex justify-between">
                ...
                <span className="font-medium text-sm">{formatPrice(Number(item.price || 0) * Number(item.quantity || 1))}</span>
              </div>
                    ...
                    option.selected_items.length > 0 && (
                        <div key={optIdx} className="text-xs text-gray-500 flex items-center">
                              ...
                              <div className="space-y-2 py-4 border-t border-b border-gray-100">
                                {console.log('[OrderSummaryContent] Antes de renderizar subtotal:', calculateTotal())}
                                ...
                                {deliveryMethod === 'delivery' && calculateShippingCost() > 0 && (
                                  <>
                                    {console.log('[OrderSummaryContent] Antes de renderizar costo de envío:', calculateShippingCost())}
                                    <div className="flex justify-between text-sm">
                              ...
                              {/* Total */}
                              {console.log('[OrderSummaryContent] Antes de renderizar total:', finalTotal)}
                                  ...
                                  <div className="flex items-center space-x-2 py-2 text-sm">
                                    {deliveryMethod === 'delivery' ? (
                                      <>
                                        ...
                                        {commerceData?.min_order_value && Number(commerceData.min_order_value) > 0 && (
                                          <span className="ml-2 text-xs bg-blue-100 text-blue-800 px-2 py-0.5 rounded-full">
                                            Pedido mínimo: {formatPrice(Number(commerceData.min_order_value))}
                                          </span>
                                        )}
                                      </>
                                    ) : (
                              ...
                              {isMobile && (
                                <div className="sticky top-[100px] z-40 px-4 mb-2">
                                  ...
                                  >
                                    ...
                                    <div className="flex items-center">
                                      {console.log('[Mobile button] Antes de renderizar total:', finalTotal)}
                                                        ...
                                                        {isOrderCompleted && whatsappRedirectUrl && (
                                                          ...
                                                          >
                                                            ...
                                                            >
                                                              ...
                                                              {/* Mostrar tiempo de entrega estimado (agregado) */}
                                                              {deliveryMethod === 'delivery' && (
                                                                <div className="bg-green-50 p-3 rounded-lg mb-4">
                                                                  <p className="text-green-800 flex items-center">
                                                                    <Truck size={18} className="mr-2" />
                                                                    <span className="font-medium">Tiempo estimado de entrega:</span>
                                                                    <span className="ml-1">{commerceData?.delivery_time || "30-60 minutos"}</span>
                                                                  </p>
                                                                </div>
                                                              )}
                                                              ...
                                                              <div className="flex gap-3 mt-6">
                                                                ...
                                                                {/* Cambio del botón "Cerrar" por "Volver a la tienda" con redirección */}
                                                                <button
                                                                  onClick={() => {
                                                                    setIsOrderCompleted(false);

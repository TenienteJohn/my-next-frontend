...
export const CheckoutPage: React.FC<CheckoutPageProps> = ({
  ...
  const generateWhatsAppLink = () => {
    ...
    return whatsappService.generateWhatsAppLink(phoneNumber, orderData);
  };
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      const orderData = {
        ...
        // Incluir el enlace de WhatsApp
        whatsappLink
      };

      // Aquí podrías realizar una petición a tu API para guardar la orden
            // const apiUrl = process.env.NEXT_PUBLIC_API_BASE_URL || 'https://tu-api-url.com';
            // const response = await axios.post(`${apiUrl}/api/orders`, orderData);

            // Si es pago con MercadoPago, podrías generar un checkout de MP y redirigir
            if (selectedPaymentMethod === 'mercado_pago') {
              // Ejemplo: Generación de checkout de MercadoPago
              // const mpResponse = await axios.post(`${apiUrl}/api/mercadopago/crear-orden`, orderData);
            ...
            // Guardar el enlace de WhatsApp para redirigir después
            ...
            // Si hay enlace de WhatsApp, redirigir después de un breve retraso
            if (whatsappLink) {
              setTimeout(() => {

...
export async function GET(request: NextRequest) {
  try {
    ...
    console.error('Proxy /tags (GET): Error en la solicitud', error);
...
export async function POST(request: NextRequest) {
  try {
    ...
    if (!body.name || !body.color || !body.type) {
      return NextResponse.json(
        { error: 'Nombre, color y tipo son obligatorios' },
        { status: 400 }
      );
    }

    // Validar tipo
    if (!['product', 'option', 'item'].includes(body.type)) {
      return NextResponse.json(
    ...
    console.error('Proxy /tags (POST): Error en la solicitud', error);
